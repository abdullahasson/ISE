...
function Container() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response1 = axios.get(`https://api.unsplash.com/search/photos?page=${MorePage}&query=${KeyWord}&client_id=${Acces}`);
                const response2 = axios.get(`https://api.unsplash.com/search/photos?page=${MorePage + 1}&query=${KeyWord}&client_id=${Acces}`);
                const response3 = axios.get(`https://api.unsplash.com/search/photos?page=${MorePage + 2}&query=${KeyWord}&client_id=${Acces}`);
    ...
    const handleScroll = () => {
        if (
            ...
            setMorePage((MorePage) => MorePage + 1);
    ...
    const showdata = dataImg.map(mg => <Link to="/ISE/zoom/"><Img sss={mg.urls.full} idA={mg.id} /></Link>)
    const showdatat = dataImgt.map(mg => <Link to="/ISE/zoom/"><Img sss={mg.urls.full} idA={mg.id} /></Link>)
    const showdatath = dataImgth.map(mg => <Link to="/ISE/zoom/"><Img sss={mg.urls.full} idA={mg.id} /></Link>) 
